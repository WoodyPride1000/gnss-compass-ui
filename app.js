...
centerOnSelf = true;
map.setView(selfLatLng, map.getZoom());
});
...
const socket = io();
socket.on('gnss', (data) => {
selfLatLng = \[data.lat, data.lng];
headingDeg = data.heading;
window\.selfAltitude = data.alt;
document.getElementById('gpsStatus').textContent = data.fix === '0' ? 'NO FIX' : 'FIX';
updateSelfPosDisplay();
animateMarkerTo(selfLatLng);
updateHeadingVisuals();
if (centerOnSelf) map.setView(selfLatLng);
});
